...
| **(emojiClick)**            |                           | not triggered on return key in search bar. Params: `{ emoji, $event }`. Always dispatched outside of the Angular zone, see [the note above](#emoji).               |
| **(emojiSelect)**           |                           | whenever an emoji is selected. returns `{ emoji, $event }`. Always dispatched outside of the Angular zone, see [the note above](#emoji).                            |
...
| **isNative**                                 |          | `false`                   | Renders the native unicode emoji                                                                                 |
| **(emojiClick)**                             |          |                           | Params: `{ emoji, $event }`. Always dispatched outside of the Angular zone, see the note below.                 |
| **(emojiLeave)**                             |          |                           | Params: `{ emoji, $event }`. Always dispatched outside of the Angular zone, see the note below.                 |
| **(emojiOver)**                              |          |                           | Params: `{ emoji, $event }`. Always dispatched outside of the Angular zone, see the note below.                 |
...
`(emojiClick)`, `(emojiLeave)` and `(emojiOver)` are always dispatched outside of the Angular zone, so hovering or clicking an emoji that nobody listens to never runs change detection. If your handler changes state that a `zone.js` app needs to redraw, call `ChangeDetectorRef.markForCheck()` (or use signals) yourself.
